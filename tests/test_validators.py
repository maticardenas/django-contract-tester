...
def test_type_validation():
    ...
    for schema, response in zip(
        example_schema_types, example_response_types, strict=False
    ):
...
def test_min_and_max_length_validation():
    ...
    with pytest.raises(
        DocumentationError,
        match=VALIDATE_MIN_LENGTH_ERROR.format(data="a" * 2, min_length=3),
    ):
    ...
    with pytest.raises(
        DocumentationError,
        match=VALIDATE_MAX_LENGTH_ERROR.format(data="a" * 6, max_length=5),
    ):
...
def test_min_and_max_items_validation():
    ...
    with pytest.raises(
        DocumentationError,
        match=VALIDATE_MIN_ARRAY_LENGTH_ERROR.format(
            data=r"\['string'\]", min_length=2
        ),
    ):
    ...
    with pytest.raises(
        ...
        match=VALIDATE_MAX_ARRAY_LENGTH_ERROR.format(
            data=r"\['string', 'string', 'string', 'string', 'string', 'string'\]",
            max_length=5,
        ),
    ):
...
def test_min_and_max_number_of_properties_validation():
    ...
    with pytest.raises(
        DocumentationError, match=VALIDATE_MINIMUM_NUMBER_OF_PROPERTIES_ERROR[:10]
    ):
    ...
    with pytest.raises(
        DocumentationError, match=VALIDATE_MAXIMUM_NUMBER_OF_PROPERTIES_ERROR[:10]
    ):
...
def test_additional_properties_not_allowed_by_default():
    ...
    with pytest.raises(
        ...
        match=VALIDATE_EXCESS_KEY_ERROR.format(
            http_message="response", excess_key="twoKey"
        ),
    ):
        tester.test_schema_section(schema, {"oneKey": "test", "twoKey": "test2"})
...
def test_string_dictionary_specified_as_additional_properties_allowed():
    ...
    tester.test_schema_section(
        schema, {"key_1": "value_1", "key_2": "value_2", "key_3": "value_3"}
    )
...
def test_string_dictionary_with_non_string_value_fails_validation():
    ...
    expected_error_message = VALIDATE_TYPE_ERROR.format(
        article="a", type="string", received=123
    )
    with pytest.raises(DocumentationError, match=expected_error_message):
        tester.test_schema_section(
            schema, {"key_1": "value_1", "key_2": 123, "key_3": "value_3"}
        )
...
def test_additional_properties_schema_not_validated_in_main_properties():
    ...
    with pytest.raises(DocumentationError, match=expected_error_message):
        tester.test_schema_section(
            schema, {"key_1": "value_1", "key_2": "value_2", "key_3": 123}
        )
...
def test_exclusives_validation():
    ...
    with pytest.raises(
        DocumentationError, match=VALIDATE_MINIMUM_ERROR.format(data=3, minimum=4)
    ):
        tester.test_schema_section(schema, 3)
    ...
    with pytest.raises(
        DocumentationError, match=VALIDATE_MAXIMUM_ERROR.format(data=5, maximum=4)
    ):
...
def test_multiple_of_validation():
    for num, _type in [(5, "integer"), (5, "number")]:
        ...
        with pytest.raises(
            DocumentationError,
            match=VALIDATE_MULTIPLE_OF_ERROR.format(data=num + 2, multiple=num),
        ):
...
def test_datetime_validation():
    # ISO8601 is valid
    tester.test_schema_section(
        {"type": "string", "format": "date-time"}, "2040-01-01 08:00"
    )

    # This is invalid
    with pytest.raises(DocumentationError):
        tester.test_schema_section(
            {"type": "string", "format": "date-time"}, "2040-01-01 9800"
        )
...
def test_byte_validation():
    tester.test_schema_section(
        ...
        base64.b64encode(b"test123").decode("utf-8"),
    )
...
def test_is_nullable_anyof():
    ...
    with pytest.raises(DocumentationError):
        tester.test_schema_section(
            {"anyOf": [{"type": "object"}, {"type": "string"}]}, None
        )
